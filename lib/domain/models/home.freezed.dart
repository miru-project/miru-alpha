...
class _DomainHomeState implements DomainHomeState {
  const _DomainHomeState({this.selectedTab = HomeTab.library, required  List<DomainExtensionMeta> libraryExtensions, required  List<DomainHistoryItem> historyItems, required  List<DomainFavoriteGroup> favoriteGroups, required  List<DomainFavorite> favorites, required  List<DomainDownload> activeDownloads, required  List<DomainDownload> finishedDownloads, this.isLoading = false, this.error}): _libraryExtensions = libraryExtensions,_historyItems = historyItems,_favoriteGroups = favoriteGroups,_favorites = favorites,_activeDownloads = activeDownloads,_finishedDownloads = finishedDownloads;
...
@pragma('vm:prefer-inline') @override $Res call({Object? extensions = null,Object? pinnedPackages = null,Object? query = freezed,}) {
  return _then(DomainLibrarySection(
...
class _DomainLibrarySection implements DomainLibrarySection {
  const _DomainLibrarySection({required  List<DomainExtensionMeta> extensions, required  List<String> pinnedPackages, this.query}): _extensions = extensions,_pinnedPackages = pinnedPackages;
