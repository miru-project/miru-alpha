...
void process_packets(int64_t &last_pts_video, int64_t &last_dts_video,
  ...
  if (input_video_stream)
    video_start_time =
        get_video_start_time(input_ctx, input_video_stream->index);
  int64_t audio_start_time = 0;
  if (input_audio_stream)
    audio_start_time =
        get_audio_start_time(input_ctx, input_audio_stream->index);
    ...
    if (input_video_stream &&
      ...
      if (packet.pts != AV_NOPTS_VALUE) {
        ...
        packet.pts = av_rescale_q_rnd(
            packet.pts, input_video_time_base, output_video_time_base,
      ...
      if (packet.dts != AV_NOPTS_VALUE) {
        ...
        packet.dts = av_rescale_q_rnd(
            packet.dts, input_video_time_base, output_video_time_base,
            static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
      ...
      if (packet.pts != AV_NOPTS_VALUE) {
        ...
        packet.pts = av_rescale_q_rnd(
            packet.pts, input_audio_time_base, output_audio_time_base,
            static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
      ...
      if (packet.dts != AV_NOPTS_VALUE) {
        ...
        packet.dts = av_rescale_q_rnd(
            packet.dts, input_audio_time_base, output_audio_time_base,
            static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
  ...
  if (input_video_stream && output_video_stream) {
    if (last_pts_video == 0) {
      ...
      last_pts_video = av_rescale_q_rnd(
          input_ctx->duration, AV_TIME_BASE_Q, output_video_stream->time_base,
          static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
  ...
  if (input_audio_stream && output_audio_stream) {
    if (last_pts_audio == 0) {
      // For the first file, get the duration in output timebase
      last_pts_audio = av_rescale_q_rnd(
          input_ctx->duration, AV_TIME_BASE_Q, output_audio_stream->time_base,
          static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
    ...
    if (i > 2) {
      ...
      for (unsigned int s = 0; s < input_ctx->nb_streams; s++) {
        if (input_ctx->streams[s]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
          ...
          std::cout << "Found video stream in file " << argv[i] << " at index "
                    << s << std::endl;
          break;
        }
      }
      ...
      for (unsigned int s = 0; s < input_ctx->nb_streams; s++) {
        if (input_ctx->streams[s]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
          ...
          std::cout << "Found audio stream in file " << argv[i] << " at index "
                    << s << std::endl;
          break;
        }
      }
